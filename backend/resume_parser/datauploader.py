...
def upsert_resume_in_supabase(employee_name, file_id, resume_data):
    ...
    if hasattr(result, 'error') and result.error:
        print(f"❌ Error storing resume for `{employee_name}` in Supabase: {result.error}")
    else:
        print(f"✅ Successfully stored resume for `{employee_name}` in Supabase")
    
    return employee_id

def query_employees(query_text, top_k=10):
    """Query employee data using vector similarity search."""
    # Generate embedding for the query
    query_embedding = generate_embedding(query_text)
    
    # Perform vector similarity search in Supabase
    result = supabase.rpc(
        'match_employees',  # Function name must match your Supabase RPC function
        {
    ...
    if hasattr(result, 'error') and result.error:
        print(f"❌ Error querying employees: {result.error}")
        return []
...
if __name__ == "__main__":
    ...
    for match in results:
        ...
        print(f"Role: {employee_data.get('role', ['Not provided'])}")
